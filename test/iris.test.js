...
describe('Iris.js', () => {
    ...
    return iris
      ...
      .catch((e) => {
        ...
        expect(e.message).to.includes(
          'error occurred when creating beneficiary'
        );
  ...
  it('able to updateBeneficiaries with existing/created account', () => {
    let iris = new Iris(generateConfig());
    return iris
      .updateBeneficiaries('budisusantoo', {
        name: 'Budi Susantoo',
        account: '0611101141',
        bank: 'bca',
        alias_name: 'budisusantoo',
  ...
  it('able to getBeneficiaries', () => {
    let iris = new Iris(generateConfig());
    return iris.getBeneficiaries().then((res) => {
  ...
  it('able to getPayoutDetails from above', () => {
    let iris = new Iris(generateConfig());
    return iris.getPayoutDetails(globVar.createdRefNo).then((res) => {
      // console.log(res);
      expect(res).to.have.property('status');
      expect(res.status).to.be.a('string');
  ...
  it('able to getTransactionHistory', () => {
    let iris = new Iris(generateConfig());
    return iris.getTransactionHistory().then((res) => {
      expect(res).to.be.an('array');
      if (res.length > 0) {
  ...
  it('able to getTopupChannels', () => {
    let iris = new Iris(generateConfig());
    return iris.getTopupChannels().then((res) => {
      expect(res).to.be.an('array');
  ...
  it('able to getBalance', () => {
    let iris = new Iris(generateConfig());
    return iris.getBalance().then((res) => {
  ...
  it('able to getBeneficiaryBanks', () => {
    let iris = new Iris(generateConfig());
    return iris.getBeneficiaryBanks().then((res) => {
