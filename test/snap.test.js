  ...
  it('able to set X-Override-Notification request header via exposed http_client object', () => {
    ...
    snap.httpClient.http_client.interceptors.request.use(
      function (config) {
        ...
        expect(config.headers.common['X-Override-Notification']).to.be.equals(
          customUrl
        );
        return config;
      },
    ...
    snap.createTransactionToken().catch((e) => {
      ...
      expect(snap.httpClient.http_client.defaults.headers.common).to.be.an(
        'object'
      );
      expect(
        snap.httpClient.http_client.defaults.headers.common[
...
function getFormattedTime(offsetInMilisecond = 0) {
  ...
  let formattedDateString =
    targetDate.toISOString().split('T').join(' ').split('.')[0] + ' +0000';
